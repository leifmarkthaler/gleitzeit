...
class TaskStatus(str, Enum):
    ...
    QUEUED = "queued"
    ROUTED = "routed"
    VALIDATING = "validating"  # Task is being evaluated by ValidationHandler
    EXECUTING = "executing"
    ...
    REWOUND = "rewound"
    SKIPPED = "skipped"  # Task skipped due to validation failure
    BLOCKED = "blocked"  # Task blocked by validation gate
...
# Retry configuration is now handled by StatelessRetryService
# See src/gleitzeit/core/stateless_retry_service.py
...
class Task(BaseModel):
    ...
    # retry_config: Removed - retry is now handled by StatelessRetryService
...
class TaskResult(BaseModel):
    ...
    # Handler tracking
    handler_id: Optional[str] = Field(None, description="Handler instance ID that executed the task")
    worker_id: Optional[str] = Field(None, description="Worker ID that owns the handler")
    provider_url: Optional[str] = Field(None, description="Provider/backend service URL (e.g., Ollama endpoint)")
    worker_instance_url: Optional[str] = Field(None, description="Worker instance URL (where handler/worker runs)")
