...
class OllamaProviderStreamlined(HubProvider[OllamaConfig]):
    ...
    async def execute_on_resource(
        ...
        if method == "llm/generate":
            return await self._generate(instance, params)
    ...
    async def _generate(self, instance: ResourceInstance[OllamaConfig], params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_method_requirements(self, method: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if method in ["llm/generate", "llm/chat", "llm/embeddings"]:
            model = params.get('model', self.default_model)
