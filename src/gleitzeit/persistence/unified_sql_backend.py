"""
Unified SQLAlchemy-based persistence backend for Gleitzeit

This consolidates all SQL persistence implementations into a single SQLAlchemy-based
solution with SQLite as the default engine. Supports both task/workflow persistence
and hub resource state persistence.
"""
...
# Create separate base classes for different table sets
TaskBase = declarative_base(name='TaskBase')
HubBase = declarative_base(name='HubBase')
...
class DBTask(TaskBase):
    ...
    task_metadata = Column('metadata', Text)  # JSON - renamed to avoid conflict
...
class DBWorkflow(TaskBase):
    ...
    workflow_metadata = Column('metadata', Text)  # JSON - renamed to avoid conflict
    created_at = Column(DateTime, nullable=False)
...
class DBWorkflowExecution(TaskBase):
...
class DBResourceInstance(HubBase):
...
class DBResourceMetrics(HubBase):
...
class UnifiedSQLBackend(PersistenceBackend):
    ...
    - SQLiteBackend (task/workflow persistence)
    - SQLAlchemyHubAdapter (hub resource persistence)
    ...
    def __init__(
        ...
        **engine_kwargs
        ...
        """
        Initialize unified SQL backend.
        
        Args:
            connection_string: SQLAlchemy connection string 
                             (default: sqlite+aiosqlite:///gleitzeit.db)
            task_db_path: Legacy parameter for compatibility (will create SQLite connection)
            **engine_kwargs: Additional arguments for create_async_engine
        """
        # Handle legacy task_db_path parameter
        if task_db_path and not connection_string:
            if task_db_path == ":memory:":
                connection_string = "sqlite+aiosqlite:///:memory:"
            else:
                connection_string = f"sqlite+aiosqlite:///{task_db_path}"
        ...
        self.db_path = task_db_path or "gleitzeit.db"
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # Create all tables for both Task and Hub models
            async with self.engine.begin() as conn:
                await conn.run_sync(TaskBase.metadata.create_all)
                await conn.run_sync(HubBase.metadata.create_all)
            
            self._initialized = True
            logger.info(f"Unified SQL backend initialized: {self.connection_string}")
            
        except Exception as e:
            logger.error(f"Failed to initialize unified SQL backend: {e}")
    ...
    async def shutdown(self) -> None:
        ...
        if self.engine:
            ...
            self.async_session = None
        self._initialized = False
        logger.info("Unified SQL backend shut down")
    
    # ========================================================================
    # Task/Workflow Operations (PersistenceBackend interface)
    # ========================================================================
    ...
    # ========================================================================
    # Hub Resource Operations (HubPersistenceAdapter interface)
    # ========================================================================
...
SQLiteBackend = UnifiedSQLBackend
SQLAlchemyHubAdapter = UnifiedSQLBackend