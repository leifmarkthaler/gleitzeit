"""
Stateless WebSocket Manager using Redis Event-Driven Scheduler.

Replaces persistent loops with event-driven processing triggered
by the Redis scheduler for heartbeat checks and periodic updates.
"""
...
class StatelessWebSocketManager:
    """
    Stateless WebSocket connection manager with Redis Event-Driven Scheduler.

    Features:
    - NO persistent loops - uses Redis event-driven scheduler for periodic processing
    - Connection limits (total and per-IP)
    - Rate limiting per connection
    - Redis PubSub for cross-instance broadcasting
    - Event-driven heartbeat/keepalive mechanism
    - Origin validation
    - Automatic cleanup triggered by scheduler events
    """
    ...
    def __init__(
        ...
        # Statistics
        self._connections_processed = 0
        self._heartbeats_processed = 0
        self._dead_connections_cleaned = 0

        logger.info(f"Stateless WebSocket manager initialized: instance_id={self.instance_id}, "
                   f"max_connections={self.max_connections}, "
                   f"max_per_ip={self.max_connections_per_ip}")

    async def initialize(self):
        """Initialize the WebSocket manager with scheduler integration."""
        # Initialize Redis for PubSub
        await self._initialize_redis()

        # Register with scheduler for periodic processing
        if self.scheduler:
            await self.scheduler.register_handler("websocket_cleanup", self._handle_cleanup_event)
            await self.scheduler.register_handler("websocket_heartbeat", self._handle_heartbeat_event)

            # Schedule periodic cleanup checks (every 30 seconds)
            await self.scheduler.schedule_event("websocket_cleanup", 30)

            # Schedule periodic heartbeat broadcasts (every 15 seconds)
            await self.scheduler.schedule_event("websocket_heartbeat", 15)

        logger.info("Stateless WebSocket manager initialized (event-driven, no loops)")

    async def _initialize_redis(self):
        """Initialize Redis connection for PubSub."""
        try:
            if hasattr(self.persistence, 'redis') and self.persistence.redis:
                self.redis_client = self.persistence.redis
            else:
            ...
            # Start listening task (this is acceptable - it's event-driven not polling)
            self.pubsub_task = asyncio.create_task(self._redis_listener())

            logger.info("Redis PubSub initialized for WebSocket broadcasting")
    ...
    async def _handle_cleanup_event(self, event_data: Dict[str, Any]):
        """Handle periodic cleanup event from scheduler."""
        logger.debug("Processing WebSocket cleanup event")

        # Check for dead connections
        cleaned = await self._check_dead_connections()
        self._dead_connections_cleaned += cleaned

        # Schedule next cleanup
        if self.scheduler:
            await self.scheduler.schedule_event("websocket_cleanup", 30)

        return {"dead_connections_cleaned": cleaned}

    async def _handle_heartbeat_event(self, event_data: Dict[str, Any]):
        """Handle periodic heartbeat broadcast event from scheduler."""
        logger.debug("Processing WebSocket heartbeat broadcast event")

        # Send heartbeat to all connections
        heartbeat_count = await self._send_heartbeat_broadcast()
        self._heartbeats_processed += heartbeat_count

        # Schedule next heartbeat
        if self.scheduler:
            await self.scheduler.schedule_event("websocket_heartbeat", 15)

        return {"heartbeats_sent": heartbeat_count}
    ...
    async def _check_dead_connections(self) -> int:
    ...
    async def _send_heartbeat_broadcast(self) -> int:
    ...
    async def tick(self) -> Dict[str, Any]:
        """
        Tick method for stateless processing (manual triggering).

        Returns:
            Processing statistics
        """
        # Check for dead connections
        dead_cleaned = await self._check_dead_connections()

        # Send heartbeat to active connections
        heartbeats_sent = await self._send_heartbeat_broadcast()

        return {
            "active_connections": len(self.active_connections),
            "dead_connections_cleaned": dead_cleaned,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            "instance_id": self.instance_id,
            "active_connections": len(self.active_connections),
            "connections_processed": self._connections_processed,
            "heartbeats_processed": self._heartbeats_processed,
            "dead_connections_cleaned": self._dead_connections_cleaned,
            "subscribed_channels": sum(len(subs) for subs in self.subscriptions.values()),
            "stateless": True,
            "event_driven": True,
            "has_loops": False
        }
    ...
    async def shutdown(self):
        ...
        logger.info("Stateless WebSocket manager cleaned up")