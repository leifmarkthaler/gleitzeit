...
class ExecutionEngine:
    ...
    def __init__(
        ...
        # Initialize parameter resolver service
        self.parameter_resolver = ParameterResolver(persistence) if persistence else None
        ...
        # Legacy event handlers removed - using event bus directly
    ...
    def add_event_handler(self, event_type: str, handler: Callable) -> None:
        """DEPRECATED: Use event_bus.register() directly instead"""
        logger.warning("add_event_handler() is deprecated. Use event_bus.register() instead.")
        # Legacy method kept for backward compatibility but no longer functional
    ...
    async def _resolve_task_parameters(self, task: Task) -> Dict[str, Any]:
        """Resolve parameter references in task parameters using ParameterResolver service"""
        if not self.parameter_resolver:
            logger.warning("No parameter resolver available, returning original params")
            return task.params
            
        # Pass task name mapping if available
        task_name_mapping = getattr(self, 'task_name_to_id_map', None)
        return await self.parameter_resolver.resolve_parameters(task, task_name_mapping)
