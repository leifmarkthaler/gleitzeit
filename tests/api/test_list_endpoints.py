...
class TestWorkflowListEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_list_workflows_empty(self, async_client, mock_gleitzeit_client):
        """Test listing workflows when none exist"""
        # Mock empty result from client
        mock_gleitzeit_client.list_workflows.return_value = {
            "workflows": [],
            "total": 0,
            "limit": 50,
            "offset": 0
    ...
    @pytest.mark.asyncio
    async def test_list_workflows_with_data(self, async_client, mock_gleitzeit_client):
        ...
        assert data["total"] == 2
        # API returns workflows directly from client - check for id field
        assert data["workflows"][0].get("id") == "wf-1" or data["workflows"][0].get("workflow_id") == "wf-1"
        assert data["workflows"][0]["name"] == "Test Workflow 1"
        assert data["workflows"][1].get("id") == "wf-2" or data["workflows"][1].get("workflow_id") == "wf-2"
    
    @pytest.mark.asyncio
    async def test_list_workflows_with_pagination(self, async_client, mock_gleitzeit_client):
        ...
        # API returns workflows directly from client - check for id field
        assert data["workflows"][0].get("id") == "wf-2" or data["workflows"][0].get("workflow_id") == "wf-2"
    
    @pytest.mark.asyncio
    async def test_list_workflows_with_status_filter(self, async_client, mock_gleitzeit_client):
    ...
    @pytest.mark.asyncio
    async def test_list_workflows_calculates_task_stats(self, async_client, mock_gleitzeit_client):
        """Test that workflow listing calculates task statistics"""
        # Create a mock workflow dict with task statistics
        workflow = {
            "id": "wf-with-tasks",
            ...
            "tasks_completed": 2,
            "tasks_failed": 1,
            "status": "failed"  # Has failed tasks
        }
...
class TestTaskListEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_empty(self, async_client, mock_gleitzeit_client):
        """Test listing tasks when none exist"""
        mock_gleitzeit_client.list_tasks.return_value = {
            "tasks": [],
            "total": 0,
            "limit": 100,
            "offset": 0
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_with_data(self, async_client, mock_gleitzeit_client):
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_by_workflow(self, async_client, mock_gleitzeit_client):
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_by_status(self, async_client, mock_gleitzeit_client):
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_with_pagination(self, async_client, mock_gleitzeit_client):
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_calculates_execution_time(self, async_client, mock_gleitzeit_client):
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_combined_filters(self, async_client, mock_gleitzeit_client):
        ...
        mock_gleitzeit_client.list_tasks.assert_called_with(
            workflow_id="wf-1",
            status="completed",
            limit=10,
            offset=0
