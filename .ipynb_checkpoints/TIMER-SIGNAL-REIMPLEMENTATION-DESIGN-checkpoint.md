...
### Phase 2: Add Timer Service
```python
# timers/service.py
class TimerService:
    """Service that checks for expired timers and emits events"""
    ...
    async def run(self):
        while True:
            ...
            expired = await self.redis.zrangebyscore(
                "timers:pending", 0, now, withscores=True
            )

            for timer_key, score in expired:
                # Extract task_id
                task_id = timer_key.split(":")[-1]

                # Emit timer expired event
                await self._emit_timer_expired(task_id)
...
1. **Week 1**: Implement basic providers without blocking
2. **Week 2**: Add timer service and polling mechanism
3. **Week 3**: Add signal API endpoints
4. **Week 4**: Full integration with StreamWorker
